...
def process_dataset(
    ...
    for roi_idx, roi in enumerate(roi_names, start=1):
        ...
        metadata = {
            ...
            "reference_shape": [int(v) for v in reference_img.shape[:3]],
            "reference_zooms": [float(v) for v in reference_img.header.get_zooms()[:3]],
            "reference_affine": reference_img.affine.astype(float).tolist(),
            "flat_voxel_count": int(np.prod(reference_img.shape[:3])),
            "max_decoder_voxel_index": int(flat_indices.max()) if flat_indices.size else None,
