...
def test_t1_truncation_keeps_decoder_grid_for_scoring(tmp_path):
    trans_dir = tmp_path / "sub-1" / "1" / "func" / "trans"
    anat_dir = tmp_path / "sub-1" / "anat"
    trunc_dir = trans_dir / "truncated_t1_refs"
    trans_dir.mkdir(parents=True)
    anat_dir.mkdir(parents=True)
    trunc_dir.mkdir(parents=True)

    (trans_dir / "epi_mask_mean.nii").touch()
    (trans_dir / "epi2t1_Composite.h5").touch()

    affine = np.eye(4)
    t1_path = anat_dir / "T1_N4.nii"
    warped_mask_path = trunc_dir / "epi_mask_mean_in_t1.nii"
    decoder_path = tmp_path / "decoder_150_grid.nii"

    rt_pipeline.nib.save(
        rt_pipeline.nib.Nifti1Image(np.ones((5, 5, 5), dtype=np.float32), affine),
        str(t1_path),
    )
    mask = np.zeros((5, 5, 5), dtype=np.float32)
    mask[1:4, 1:4, 1:4] = 1
    rt_pipeline.nib.save(rt_pipeline.nib.Nifti1Image(mask, affine), str(warped_mask_path))
    rt_pipeline.nib.save(
        rt_pipeline.nib.Nifti1Image(np.ones((3, 3, 3), dtype=np.float32), affine),
        str(decoder_path),
    )

    cfg = type("Cfg", (), {})()
    cfg.trans_dir = trans_dir
    cfg.subject_root = tmp_path / "sub-1"
    cfg.decoder_template = decoder_path
    cfg.enable_scoring = True

    old_truncate = rt_pipeline.REGRESSOR_SETTINGS.truncate_t1_to_epi_fov
    try:
        rt_pipeline.REGRESSOR_SETTINGS.truncate_t1_to_epi_fov = True
        t1_reference = rt_pipeline.maybe_prepare_truncated_t1_reference(cfg)
    finally:
        rt_pipeline.REGRESSOR_SETTINGS.truncate_t1_to_epi_fov = old_truncate

    assert t1_reference is None
    assert cfg.decoder_template == decoder_path


def test_t1_output_reference_uses_decoder_grid_when_scoring(tmp_path):
    cfg = type("Cfg", (), {})()
    cfg.base_data = tmp_path
    cfg.decoder_template = tmp_path / "decoder_150_grid.nii"
    cfg.t1_reference_override = tmp_path / "T1_208_grid.nii"

    cfg.enable_scoring = True
    assert rt_pipeline.resolve_t1_output_reference(cfg) == cfg.decoder_template

    cfg.enable_scoring = False
    assert rt_pipeline.resolve_t1_output_reference(cfg) == cfg.t1_reference_override
