...
%   dll         The library file to use.  Specify an absolute path or make 
%               sure the location of the file is in the search path. Use
%               the "which" command if not specifying an absolute path and
%               needing to ensure the correct library is used.
%   dothdir     directory where mdpev.h is located
%   mptype      enumerated value for MP device. Look inside mpdev.h to see 
%               the options and their enumeration.
%   mpmethod    enumerated value for MP communication method, either MPUSB
%               or MPUDP.  
%   sn          Serial Number of the MP device.  Not used for all hardware
%               types (generally not used for devices with USB
%               communication). 
...
if nargin ~= 5
    error('MPDEVDEMO requires 5 arguments');
end

if isnumeric(dll) || isnumeric(dothdir)
    error('DLL and Header Directory have to be strings')
end
...
loadlibrary(dll,@mpdevproto2024);

%in case of failure, comment the above line and uncomment the next line
%to generate thunk dll and prototype files in the current folder.  Note
%that the thunk file included with the distribution was created with the
%64-bit version of mpdev.dll created in Visual Studio 2019.  A new thunk
%file may be required if using a different version of the library.

%loadlibrary(dll,strcat(dothdir,doth), 'mfilename', 'mpdevproto2024');

%or use the .h file from BHAPI
...
function retval = startAcquisitionDemo(dothdir, libname,mptype, mpmethod, sn)
    ...
    switch mptype 
        case { 101, 103, 104 }   % MP150, MP160, or MP200
        ...
        case { 102, 105 } % MP36R or MP36AR
            aCH = [int32(1),int32(1),int32(1),int32(0)];
        otherwise
            fprintf(1,'Unknown hardware type.\n')
            callib(libname, 'disconnectMPDev')
            return
    end
